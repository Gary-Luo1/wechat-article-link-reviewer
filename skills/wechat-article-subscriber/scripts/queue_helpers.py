...
def get_processed_entry(link: str) -> dict[str, Any] | None:
    """Return a processed entry by stable URL without any network access."""
    normalized = normalize_url(link)
    with queue_lock():
        entry = _read_unlocked()["processed"].get(normalized)
...
def add_pending_with_verified_read(
    source: dict[str, Any],
    text: str,
    *,
    content_dedup: bool = False,
) -> tuple[str, dict[str, Any]]:
    """Atomically queue one directly supplied article with full-read proof."""
    ...
    with queue_lock():
        data = _read_unlocked()
        processed = data["processed"].get(normalized)
        if isinstance(processed, dict):
            return "already_processed", deepcopy(processed)
        existing = next(
            (
                item
                for item in data["pending"]
        ...
        if existing is not None:
            ...
            if digest:
                ...
                existing.pop("content_hash", None)
            existing["read_state"] = read_state
            _write_unlocked(data)
            return "already_pending", deepcopy(existing)
        if content_dedup and digest:
            known_hashes = {
                item.get("content_hash")
                for item in data["pending"]
                if item.get("content_hash")
            ...
            if digest in known_hashes:
                return "duplicate_content", article
        article["id"] = hashlib.sha256(normalized.encode("utf-8")).hexdigest()[:16]
        article["normalized_url"] = normalized
        if digest:
            article["content_hash"] = digest
        ...
        data["pending"].append(article)
        _write_unlocked(data)
        return "queued", deepcopy(article)
